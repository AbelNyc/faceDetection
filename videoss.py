...
def run():
    ...
    # track movement changes,,if object is showing in the webcam
    movement_list = [None, None]
    ...
    while True:
        ...
        threshold_binary = cv2.threshold(cur_frame, 30, 255, cv2.THRESH_BINARY)[
            1]  # only need to access the second item of the tuple
        threshold_binary = cv2.dilate(threshold_binary, None, iterations=2)
        #find the countors
        (cnts, _) = cv2.findContours(threshold_binary.copy(),
        ...
        if cv2.waitKey(1) == ord('s'):  # s to quit
            if movement == 1:
                time.append(datetime.now())  # trakc exit time
            break
    print(movement_list)
    print(time)
